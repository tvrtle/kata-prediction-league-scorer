...
final class ScorelineCompleteB
{
    ...
    public static function from(array|string $input): self
    {
        if (is_array($input) && count($input) === 2) {
            if (!is_numeric($input[0]) || !is_numeric($input[1])) {
                throw new InvalidArgumentException("Scoreline array must contain numeric values");
            }
        ...
        if (is_string($input)) {
            ...
            throw new InvalidArgumentException("Invalid string format: {$input}");
        }
